...
import { normalizeGithubDelivery, readWebhookSecret, verifyGithubSignature } from "./github.js";
...
describe("GitHub webhooks", () => {
  ...
  it("never accepts an empty webhook secret, even when the signature was made with one", () => {
    const body = Buffer.from('{"ok":true}');
    const forged = sign(body.toString(), "");
    expect(verifyGithubSignature(body, forged, "")).toBe(false);
    expect(verifyGithubSignature(body, forged, "   ")).toBe(false);
    expect(() => readWebhookSecret({ GITHUB_WEBHOOK_SECRET: "" })).toThrow(/GITHUB_WEBHOOK_SECRET/);
    expect(() => readWebhookSecret({})).toThrow(/GITHUB_WEBHOOK_SECRET/);
    expect(() =>
      createApp({
        pool: {} as Pool,
        webhookSecret: "",
